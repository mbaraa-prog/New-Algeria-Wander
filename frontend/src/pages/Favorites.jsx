  ...
  const filteredFavorites = useMemo(() => {
    ...
    return favorites.filter(item => {
      ...
      if (activeFilter === 'Hotels')
        return t.includes('hotel') || t.includes('accommodation') || t.includes('stay') || t.includes('lodge');
      if (activeFilter === 'Landmarks')
        return t.includes('landmark') || t.includes('attraction') || t.includes('destination') || t.includes('site') || t.includes('place') || t.includes('park') || t.includes('museum');
      if (activeFilter === 'Restaurants')
        return t.includes('restaurant') || t.includes('dining') || t.includes('cafe') || t.includes('food');
      return true;
    });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-4 mb-12 overflow-x-auto no-scrollbar py-2">
          {filters.map(filter => (
            <button
              ...
              className={`px-8 py-2.5 rounded-full text-sm font-bold transition-all shadow-sm ${activeFilter === filter
        ...
        {filteredFavorites.length > 0 ? (
          ...
          <div className="bg-white rounded-[40px] p-24 text-center shadow-sm border border-gray-100">
            ...
            <h3 className="text-[#0F4C81] text-2xl font-bold mb-4">
              {activeFilter === 'All' ? 'No favorites yet' : `No ${activeFilter.toLowerCase()} in favorites`}
            </h3>
            <p className="text-gray-400 max-w-sm mx-auto mb-10">
              {activeFilter === 'All'
                ...
                : `You have not saved any ${activeFilter.toLowerCase()} yet. Click the heart icon on any place to save it.`}
            </p>
