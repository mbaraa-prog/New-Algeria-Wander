...
const PlaceCard = ({ item, type = 'default', onRemove }) => {
  ...
  if (type === 'event') {
    return (
      <Link to={`/details/${item.id}`} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 block group relative">
        <button 
      ...
      <Link to={`/details/${item.id}`} className="block">
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[#FF7F50] text-xs font-bold uppercase tracking-wider">{item.type}</span>
            <div className="flex items-center text-gray-500 text-sm">
